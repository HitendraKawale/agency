...
export interface TerminalTextRevealProps {
  ...
  accentColor?: string;
  finalColor?: string;
  /** Root surface colors (defaults: black field, white type). */
      ...
      style={
        {
          ...
          "--ttr-bg": backgroundColor,
          "--ttr-fg": foregroundColor,
...
.ttr-root {
  width: 100%;
  background: var(--ttr-bg, #0a0a0a);
  color: var(--ttr-fg, #f3f3f1);
  font-family: var(--font-rethink-sans), "Geist", "Inter", system-ui, sans-serif;
}
...
.ttr-image img {
  border-radius: 8px;
  box-shadow: 0 0 0 1px color-mix(in srgb, var(--ttr-fg, #f3f3f1) 8%, transparent);
}
...
.ttr-rule {
  ...
  width: 1px;
  height: 4rem;
  background: color-mix(in srgb, var(--ttr-fg, #f3f3f1) 18%, transparent);
}

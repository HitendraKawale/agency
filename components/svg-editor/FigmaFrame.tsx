...
export function FigmaFrame({
  ...
  width,
  height,
  /** Skip the intro animation entirely (reduced motion). */
  animate = true,
}: {
  children: ReactNode;
  style?: FrameStyle;
  ...
  useEffect(() => {
    // Explicit size wins below (`width ?? size.w`), so there's nothing to
    // measure and nothing to store — observing would only cascade a render.
  ...
  const handle = (
    ...
    ...(intro
      ? {
          animation: `bbox-handle 0.25s ease-out both`,
  ...
  const intro = animate && firstMount;
      ...
      {s.showBadge && (
        <span
          ...
          style={{
            ...
            ...(intro
              ? {
                  animation: `bbox-badge 0.25s ease-out both`,
                  animationDelay: "0.2s",
                }
              : {}),
          }}

...
// SUBPATHS (a shape with a hole is an outer ring plus an inner ring). Anchors
// are kept in ONE flat array; `starts` marks where each subpath begins and
// `closed` is the per-subpath Z flag. Serializing back to a path `d` string is
// lossless for the cubic/line subset we support.
...
export function subRange(path: VectorPath, s: number): [number, number] {
  ...
  const end =
    s + 1 < path.starts.length ? path.starts[s + 1] : path.anchors.length;
  return [begin, end];
}

...
// Arcs (A) are not control-point editable, so a path using them is rejected.
// Everything is normalized to cubic beziers internally so the editor only ever
// deals with anchors + two handles.
//
...
export function parsePath(d: string): VectorPath {
  ...
  // its `in` handle onto the subpath's first anchor.
  const dedupClose = () => {
    ...
    switch (C) {
      ...
      case "S": {
        while (i < args.length) {
          const c1: Vec =
            prevCmd === "C" || prevCmd === "S"
              ? {
                  x: 2 * cur.x - (prevCtrl?.x ?? cur.x),
                  y: 2 * cur.y - (prevCtrl?.y ?? cur.y),
                }
              : cur;
          const c2 = pt();
          const end = pt();
      ...
      case "Q": {
        while (i < args.length) {
          ...
          const end = pt();
          const c1 = {
            x: cur.x + (2 / 3) * (qc.x - cur.x),
            y: cur.y + (2 / 3) * (qc.y - cur.y),
          };
          const c2 = {

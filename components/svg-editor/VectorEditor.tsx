...
export function VectorEditor({
  ...
  // Mirrored in an effect rather than during render: the effect commits before
  // paint, so the rAF callback — which fires after paint — always sees it.
  ...
  function applyDrag(d: Drag, u: Vec): VectorPath {
    ...
    if (d.kind === "anchor") {
      ...
      if (d.mirror !== "none" && other) {
        ...
        const len =
          d.mirror === "angle-length"
            ? relLen
            : Math.hypot(other.x - a.p.x, other.y - a.p.y);
        const opp = {
          x: a.p.x - (rel.x / relLen) * len,
          y: a.p.y - (rel.y / relLen) * len,
        };
